...
uint64_t TCPSender::bytes_in_flight() const { return _next_seqno - _send_base_seqno; }
...
void TCPSender::fill_window() {
    ...
    while (window_remain > 0 && !_stream.buffer_empty()) {
        TCPSegment seg;
        seg.payload() = Buffer(_stream.read(std::min(TCPConfig::MAX_PAYLOAD_SIZE, window_remain)));
...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    while (!_segments_wait.empty()) { // 弹出ackno之前的所有segment
        ...
        if (unwrap(seg.header().seqno, _isn, _next_seqno) + seg.length_in_sequence_space() > abs_ack) {
            break;
        }
        _segments_wait.pop();
...
void TCPSender::tick(const size_t ms_since_last_tick) { 
    ...
    if (_ticks >= _rto_timeout && _segments_wait.size() > 0) {
        ...
        _segments_out.push(seg);
        _ticks = 0;
        // TEST: When filling window, treat a '0' window size as equal to '1' but don't back off RTO"
        // syn acked之后才传有window_size
